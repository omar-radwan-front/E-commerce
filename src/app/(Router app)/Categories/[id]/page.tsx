import OneCategory from '@/Api/oneCategory';
...
export default async function CategorytDetails({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;

  const data = await OneCategory(id);
  console.log(data);
  
  if (!data) return <h1 className="text-3xl text-center text-red-600">No Category</h1>;
